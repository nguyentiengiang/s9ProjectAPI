    ...
    private static function _getPassedNames($trace) {
        ...
        while ($callee = array_pop($trace)) {
            if (strtolower($callee['function']) === 'd' ||
                    ...
                    ( isset($callee['class']) && strtolower($callee['class']) === strtolower(__CLASS__) && strtolower($callee['function']) === 'dump' )
            ) {
                break;
            } else {
        ...
        $codePattern = empty($callee['class']) ? $callee['function'] : $callee['class'] . "\x07*" . $callee['type'] . "\x07*" . $callee['function'];
